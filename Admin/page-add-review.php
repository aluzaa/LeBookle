...
// Ambil data buku
$books = $conn->query("SELECT id, judul FROM books");
if (!$books) {
    die("Error fetching books: " . $conn->error);
}

// Proses form untuk menambah review
if ($_SERVER["REQUEST_METHOD"] == "POST") {
    ...
    // Query untuk menyimpan data review ke database
    $sql = $conn->prepare("INSERT INTO reviews (user_id, book_id, rating, comment, created_at) VALUES (?, ?, ?, ?, NOW())");
    $sql->bind_param("iiis", $user_id, $book_id, $rating, $comment);

    // Eksekusi query
    if ($sql->execute()) {
        // Redirect ke page-list-review.php setelah berhasil
        header("Location: page-list-review.php");
        exit;
    } else {
...
<html lang="en">
    ...
    <div class="card-header">
        <h4 class="card-title">Tambah Review</h4>
    </div>
    <div class="card-body">
        <form action="page-add-review.php" method="POST">
            <div class="form-group">
                <label for="user_id">User *</label>
                <select name="user_id" class="form-control" required>
                    <?php
                    // Menampilkan dropdown pilihan user
                    if ($users->num_rows > 0) {
                        while ($user = $users->fetch_assoc()) {
                            echo "<option value='" . $user['id'] . "'>" . htmlspecialchars($user['username']) . "</option>";
                        }
                    } else {
                        echo "<option value=''>Tidak ada user ditemukan</option>";
                    }
                    ?>
                </select>
            </div>
            <div class="form-group">
                ...
                <select name="book_id" class="form-control" required>
                    <?php
                    // Menampilkan dropdown pilihan buku
                    if ($books->num_rows > 0) {
                        while ($book = $books->fetch_assoc()) {
                            echo "<option value='" . $book['id'] . "'>" . htmlspecialchars($book['judul']) . "</option>";
                        }
                    } else {
                        echo "<option value=''>Tidak ada buku ditemukan</option>";
                    }
                    ?>
                </select>
            </div>
