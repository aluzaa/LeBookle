...
$sql = "SELECT id, title, created_at, cover_image, quote, quote_author FROM blog";
...
<html lang="en">
    ...
    <div class="card-header d-flex justify-content-between align-items-center">
        <div class="header-title">
            <h4 class="card-title mb-0">Daftar Artikel Blog</h4>
        </div>
        <div class="user-list-files d-flex">
            <a href="page-add-blog.php" class="btn btn-primary add-list mr-2"><i class="las la-plus mr-2"></i>Tambah Artikel</a>
    ...
    <div class="card-body">
        <div class="container mt-4">
            ...
            <div class="table-responsive">
                <table id="blog-list-table" class="table table-striped dataTable mt-4" role="grid" aria-describedby="blog-list-page-info">
                    <thead>
                        <tr class="ligth">
                            <th>No<i class="sort-icon"></i></th>
                            <th>Gambar Sampul<i class="sort-icon"></i></th>
                            <th>Judul<i class="sort-icon"></i></th>
                ...
                ?>
                    <tr>
                        <td><?php echo $no++; ?></td> <!-- Menggunakan variabel $no -->
                                    <td>
                                        <?php if ($row['cover_image']): ?>
                                            <img src="<?php echo htmlspecialchars($row['cover_image']); ?>" alt="Cover Image" style="width: 100px; height: auto;">
                                        <?php else: ?>
                                            Tidak ada gambar
                                        <?php endif; ?>
                                    </td>
                                    ...
                                    <td>
                                        <div class="flex align-items-center list-user-action">
                                            <a class="btn btn-sm bg-primary view-blog-btn" 
                                            data-toggle="tooltip" 
                                            data-placement="top" 
                                            title="View" 
                                            href="view_blog.php?id=<?php echo $row['id']; ?>">
                                                <i class="ri-eye-line mr-0"></i>
                                            </a>
                                            <a href="update_blog.php?id=<?php echo $row['id']; ?>&title=<?php echo urlencode($row['title']); ?>" class="btn btn-sm bg-success">
                                                <i class="ri-pencil-line mr-0"></i>
                                            </a>
                                            <a href="javascript:void(0);" 
                                            class="btn btn-sm bg-warning delete-blog" 
                        ...
                        <?php else: ?>
                            <tr>
                                <td colspan="7">Tidak ada artikel yang ditemukan.</td>
                            </tr>
                        <?php endif; ?>
                    </tbody>
                </table>
...
<script>
    document.addEventListener("DOMContentLoaded", function () {
        const table = document.getElementById("blog-list-table");
    ...
    paginate();
        ...
        // Listen to entries select changes
        ...
        nextPageButton.addEventListener("click", () => {
            currentPage = Math.min(
                Math.ceil(getFilteredRows().length / rowsPerPage),
                currentPage + 1
            );
            renderTable();
        });
...
<script>
    ...
    button.addEventListener('click', function() {
        ...
        fetch(`view_blog.php?id=${blogId}`)
            .then(response => response.text())
            .then(data => {
                // Populate the modal with fetched data
                document.getElementById('blogDetails').innerHTML = data;
...
document.addEventListener('DOMContentLoaded', function () {
    ...
    editModal.addEventListener('show.bs.modal', function (event) {
        ...
        // Isi data ke form di modal
        editModal.querySelector('#edit-blog-id').value = id;
        editModal.querySelector('#edit-title').value = title;
        editModal.querySelector('#edit-content').value = content;
        if (coverImage) {
            editModal.querySelector('#edit-cover-image-preview').src = coverImage;
        }
    });
});
</script>
<script>
    document.addEventListener("DOMContentLoaded", function() {
    // Menambahkan event listener pada semua tombol dengan kelas delete-blog
    document.querySelectorAll('.delete-blog').forEach(function(button) {
        button.addEventListener('click', function() {
            var blogId = this.getAttribute('data-id'); // Ambil ID blog dari atribut data-id
...
function exportToPDF() {
    ...
    // Pilih elemen tabel daftar artikel yang ingin diekspor
    const element = document.querySelector('.table-responsive'); 

    if (!element) {
        alert("Tabel artikel tidak ditemukan!");
        return;
    }
    ...
    html2canvas(element).then(canvas => {
        ...
        const imgWidth = 190; // Lebar PDF
        const imgHeight = (canvas.height * imgWidth) / canvas.width; // Tinggi disesuaikan proporsi

        // Masukkan canvas sebagai gambar ke dalam PDF
        doc.addImage(imgData, 'PNG', 10, 10, imgWidth, imgHeight);

        // Simpan file PDF dengan nama yang relevan
        doc.save("DaftarArtikel.pdf");
    }).catch(error => {
        console.error("Terjadi kesalahan saat mengekspor ke PDF:", error);
        alert("Gagal mengekspor daftar artikel ke PDF.");
    });
}
</script>
