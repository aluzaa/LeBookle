...
// Query untuk mengambil data orders dan pembayaran terkait
$sql = "SELECT o.id AS order_id, o.order_date, o.status, o.total, 
        COUNT(oi.id) AS item_count,
        p.status AS payment_status, p.amount AS payment_total, p.payment_date 
        FROM orders o
        ...
        <div class="card">
            <div class="card-header d-flex justify-content-between align-items-center">
                <div class="header-title">
                    <h4 class="card-title mb-0">Daftar Pembayaran</h4>
                </div>
                <div class="user-list-files d-flex">
                    <a href="page-add-customer.php" class="btn btn-primary add-list mr-2"><i class="las la-plus mr-2"></i>Tambah Pembayaran</a>
    ...
    <table id="order-list-table" class="table table-striped dataTable mt-4" role="grid" aria-describedby="order-list-page-info">
        <thead>
            <tr class="ligth">
                ...
                <th>Tanggal<i class="sort-icon"></i></th>
                <th>Jumlah Item<i class="sort-icon"></i></th>
                <th>Status Pembayaran<i class="sort-icon"></i></th>
                <th>Total Pembayaran<i class="sort-icon"></i></th>
                <th>Tanggal Pembayaran<i class="sort-icon"></i></th>
        ...
        <tbody>
            <?php if ($result->num_rows > 0): ?>
                <?php while ($row = $result->fetch_assoc()): ?>
                    <tr>
                        ...
                        <td><?php echo htmlspecialchars($row['payment_status'] ?? 'Belum Dibayar'); ?></td>
                        <td><?php echo "Rp " . number_format($row['payment_total'] ?? 0, 2, ',', '.'); ?></td>
            ...
            <?php else: ?>
                <tr>
                    <td colspan="9">Tidak ada pesanan yang ditemukan.</td>
                </tr>
            <?php endif; ?>
        </tbody>
    </table>
