...
$sql = "SELECT id, judul, penulis, penerbit, tahun_terbit, genre, halaman, berat, sinopsis, kondisi, price, stock, created_at, kategori, isbn, cover FROM books";
...
<html lang="en">
        ...
        <div class="card">
            <div class="card-header d-flex justify-content-between align-items-center">
                ...
                <div class="user-list-files d-flex">
                    <a href="page-add-product.php" class="btn btn-primary add-list mr-2"><i class="las la-plus mr-2"></i>Tambah Produk</a>
    ...
    <table id="book-list-table" class="table table-striped dataTable mt-4" role="grid" aria-describedby="book-list-page-info">
        <thead>
            <tr class="ligth">
                <th>No.<i class="sort-icon"></i></th>
                <th>Cover<i class="sort-icon"></i></th>
                <th>Judul<i class="sort-icon"></i></th>
                <th>Penulis<i class="sort-icon"></i></th>
                <th>Harga<i class="sort-icon"></i></th>
                <th>Stok<i class="sort-icon"></i></th>
                    ...
                    <tr>
                        ...
                        <td>
                            <?php if (!empty($row['cover'])): ?>
                                <img src="<?php echo htmlspecialchars($row['cover']); ?>" alt="Cover" style="max-width: 100px; max-height: 100px;">
                            <?php else: ?>
                                <span>No Cover</span>
                            <?php endif; ?>
                        </td>
                        ...
                        <td>
                            <div class="flex align-items-center list-user-action">
                                <a href="view_product.php?id=<?php echo $row['id']; ?>" class="btn btn-sm bg-primary view-book-btn" data-toggle="tooltip" data-placement="top" title="View">
                                    <i class="ri-eye-line mr-0"></i>
                                </a>
                                <a href="update_product.php?id=<?php echo $row['id']; ?>" class="btn btn-sm bg-success">
                                    <i class="ri-pencil-line mr-0"></i>
                                </a>
                                <a href="javascript:void(0);" class="btn btn-sm bg-warning delete-book" data-id="<?php echo $row['id']; ?>" title="Delete">
...
<script>
    document.addEventListener("DOMContentLoaded", function () {
        const table = document.getElementById("book-list-table"); // Update to match your table ID
...
<script>
    document.addEventListener("DOMContentLoaded", function () {
        // Menambahkan event listener pada semua tombol dengan kelas delete-book
        document.querySelectorAll('.delete-book').forEach(function (button) {
            button.addEventListener('click', function () {
                var bookId = this.getAttribute('data-id'); // Ambil ID buku dari atribut data-id
...
function exportToPDF() {
    ...
    // Pilih elemen tabel daftar produk yang ingin diekspor
    ...
    html2canvas(element).then(canvas => {
        ...
        doc.save("DaftarProduk.pdf");
    }).catch(error => {
        console.error("Terjadi kesalahan saat mengekspor ke PDF:", error);
        alert("Gagal mengekspor daftar produk ke PDF.");
    });
}
</script>
