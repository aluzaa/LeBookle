...
// Proses form untuk menambah data pelanggan baru
if ($_SERVER["REQUEST_METHOD"] == "POST") {
    ...
    // Siapkan query
    $sql = $conn->prepare("INSERT INTO customer (user_id, fullname, address, city, state, postal_code, country, phone) VALUES (?, ?, ?, ?, ?, ?, ?, ?)");
    if (!$sql) {
        die("SQL Error: " . $conn->error);
    }

    // Bind parameter dan eksekusi
    $sql->bind_param("isssssss", $user_id, $fullname, $address, $city, $state, $postal_code, $country, $phone);
    if ($sql->execute()) {
        header("Location: page-list-customer.php"); // Redirect ke halaman daftar pelanggan
    } else {
...
<html lang="en">
    ...
    <div class="card-body">
        <form action="page-add-customer.php" method="POST">
            <!-- User ID -->
