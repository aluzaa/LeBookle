...
if ($_SERVER["REQUEST_METHOD"] == "POST") {
    ...
    if (isset($_FILES['cover_image']) && $_FILES['cover_image']['error'] == 0) {
        ...
        if (in_array($fileType, $allowedTypes)) {
            ...
            if (!move_uploaded_file($_FILES['cover_image']['tmp_name'], $cover_image)) {
                echo "Error: Gagal meng-upload gambar sampul.";
                exit;
            }
        }
    }

    // Default ke gambar sampul lama jika tidak ada gambar baru
    $cover_image = $cover_image ?? $row['cover_image'];

    // Periksa upload gambar header
    if (isset($_FILES['header_image']) && $_FILES['header_image']['error'] == 0) {
        $fileType = mime_content_type($_FILES['header_image']['tmp_name']);
        $allowedTypes = ['image/jpeg', 'image/png', 'image/gif', 'image/webp'];

        if (in_array($fileType, $allowedTypes)) {
            $uniqueFileName = time() . '_header_' . basename($_FILES['header_image']['name']);
            $header_image = 'uploads/' . $uniqueFileName;

            if (!move_uploaded_file($_FILES['header_image']['tmp_name'], $header_image)) {
                echo "Error: Gagal meng-upload gambar header.";
                exit;
            }
        }
    }

    // Default ke gambar header lama jika tidak ada gambar baru
    $header_image = $header_image ?? $row['header_image'];
    ...
    $updateSql->bind_param("ssssssi", $title, $content, $cover_image, $header_image, $quote, $quote_author, $id);

    if ($updateSql->execute()) {
        // Redirect ke halaman daftar blog
        header("Location: page-list-blog.php");
        exit;
    } else {
...
<html lang="en">
    ...
    <div class="form-group">
        ...
        <?php if ($cover_image): ?>
            <p>Gambar Sampul saat ini: <a href="<?php echo $cover_image; ?>" target="_blank">Lihat Gambar</a></p>
        <?php endif; ?>
    </div>
    <div class="form-group">
        <label for="header_image">Gambar Header</label>
        <input type="file" name="header_image" class="form-control-file">
        <?php if (!empty($header_image)): ?>
            <p>Gambar Header saat ini: <a href="<?php echo htmlspecialchars($header_image); ?>" target="_blank">Lihat Gambar</a></p>
        <?php endif; ?>
    </div>
    <div class="form-group">
        <label for="quote">Kutipan</label>
        <textarea name="quote" class="form-control" rows="3"><?php echo htmlspecialchars($quote); ?></textarea>
    </div>
    <div class="form-group">
        <label for="quote_author">Penulis Kutipan</label>
        <input type="text" name="quote_author" class="form-control" value="<?php echo htmlspecialchars($quote_author); ?>">
    </div>
