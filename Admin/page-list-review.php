...
// Menjalankan query
$sql = "SELECT r.id, u.username, b.judul AS book_title, r.book_id, r.rating, r.comment, r.created_at
        FROM reviews r
        ...
        <div class="card">
            <div class="card-header d-flex justify-content-between align-items-center">
                <div class="header-title">
                    <h4 class="card-title mb-0">Daftar Review</h4>
                </div>
                <div class="user-list-files d-flex">
                    <a href="page-add-review.php" class="btn btn-primary add-list mr-2"><i class="las la-plus mr-2"></i>Tambah Review</a>
            ...
            <div class="d-flex justify-content-between align-items-center mb-3">
                ...
                <div class="table-responsive">
                    <table id="review-list-table" class="table table-striped dataTable mt-4" role="grid" aria-describedby="review-list-page-info">
                        <thead>
                            <tr class="ligth">
                                <th>Id<i class="sort-icon"></i></th>
                                <th>Username<i class="sort-icon"></i></th>
                                <th>Judul Buku<i class="sort-icon"></i></th>
                                <th>Rating<i class="sort-icon"></i></th>
                                <th>Komentar<i class="sort-icon"></i></th>
                                <th>Dibuat pada<i class="sort-icon"></i></th>
                                ...
                                <?php while ($row = $result->fetch_assoc()): ?>
                                    <tr>
                                        ...
                                        <td>
                                            ...
                                            <a class="btn btn-sm bg-primary view-review-btn" 
                                                data-toggle="tooltip" 
                                                data-placement="top" 
                                                title="View" 
                                            ...
                                            <a class="btn btn-sm bg-success" data-bs-toggle="modal" data-bs-target="#editReviewModal" 
                                                data-id="<?php echo $row['id']; ?>" 
                                                data-book_id="<?php echo $row['book_id']; ?>" 
                                                data-rating="<?php echo htmlspecialchars($row['rating']); ?>" 
                                                data-comment="<?php echo htmlspecialchars($row['comment']); ?>">
                                                <i class="ri-pencil-line mr-0"></i>
                                            </a>
                                            <a href="javascript:void(0);" 
                                                class="btn btn-sm bg-warning delete-review" 
                                                data-id="<?php echo $row['id']; ?>" 
                                                title="Delete">
    ...
    <div class="modal-dialog modal-lg" role="document">
        <div class="modal-content">
            <div class="modal-header">
                <h5 class="modal-title" id="viewReviewModalLabel">Detail Review</h5>
            </div>
            <div class="modal-body" id="reviewDetails">
                <!-- Detail review dan cover buku akan dimuat di sini melalui AJAX -->
...
<div class="modal fade" id="editReviewModal" tabindex="-1" aria-labelledby="editReviewModalLabel" aria-hidden="true">
    ...
    <div class="modal-body">
        ...
        <div class="form-group">
            ...
            <textarea class="form-control" id="edit-comment" name="comment" rows="3" required></textarea>
        </div>
    </div>
    <div class="modal-footer">
...
<!-- Inisialisasi DataTables -->
    ...
    const table = document.getElementById("review-list-table"); // ID tabel review
    ...
    const sortedRows = rows.sort((a, b) => {
        ...
        // Coba parsing nilai sebagai tanggal
        ...
        // Jika bukan tanggal, cek apakah numerik
    ...
    function renderTable() {
        ...
        paginationInfo.textContent = `Menampilkan ${showingStart} sampai ${showingEnd} dari ${totalRows} review`;
...
<script>
    ...
    // Event listener untuk tombol "View Review"
document.querySelectorAll('.view-review-btn').forEach(button => {
    button.addEventListener('click', function() {
        const reviewId = this.getAttribute('data-id');

        // Ambil data review menggunakan AJAX
        fetch(`view_review.php?id=${reviewId}`)
            .then(response => response.text())
            .then(data => {
                // Isi modal dengan data yang diterima
                document.getElementById('reviewDetails').innerHTML = data;
...
document.addEventListener('DOMContentLoaded', function () {
    ...
    editReviewModal.addEventListener('show.bs.modal', function (event) {
        ...
        const bookId = button.getAttribute('data-book_id');  // Ambil book_id dari tombol
        const rating = button.getAttribute('data-rating');
        const comment = button.getAttribute('data-comment');
        
        // Isi formulir dengan data yang sesuai
        editReviewModal.querySelector('#edit-review-id').value = reviewId;
        editReviewModal.querySelector('#edit-book-id').value = bookId; // Isi input hidden
        editReviewModal.querySelector('#edit-rating').value = rating;
        editReviewModal.querySelector('#edit-comment').value = comment;
    });
});
...
document.addEventListener("DOMContentLoaded", function() {
    document.querySelectorAll('.delete-review').forEach(function(button) {
        button.addEventListener('click', function() {
            var reviewId = this.getAttribute('data-id'); // Ambil review ID dari data-id
            console.log("Review ID yang akan dihapus:", reviewId); // Log ID untuk pengecekan
            ...
            if (confirm('Apakah Anda yakin ingin menghapus review ini?')) {
                ...
                .then(data => {
                    if (data.success) {
                        alert(data.message); 
                        location.reload(); // Reload halaman untuk memperbarui daftar review
                    } else {
                        alert(data.message); 
                    }
                })
