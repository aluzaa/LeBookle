...
// Proses form untuk menambah artikel
if ($_SERVER["REQUEST_METHOD"] == "POST") {
    $title = $_POST['title'];
    $content = $_POST['content'];
    $quote = $_POST['quote']; // Tangkap input kutipan
    $quote_author = $_POST['quote_author']; // Tangkap input penulis kutipan
    $cover_image = null;

    // Memeriksa jika gambar cover ada
    ...
    // Tambahkan di atas query untuk menyimpan artikel
$header_image = null;

// Memeriksa jika gambar header ada
...
// Tambahkan kolom 'header_image' ke query untuk menyimpan artikel
$sql = $conn->prepare("INSERT INTO blog (title, content, cover_image, header_image, quote, quote_author, created_at) VALUES (?, ?, ?, ?, ?, ?, NOW())");
$sql->bind_param("ssssss", $title, $content, $cover_image, $header_image, $quote, $quote_author);

    if ($sql->execute()) {
        // Redirect ke page-list-blog.php jika berhasil
...
<html lang="en">
    ...
    <div class="card-body">
    <form action="page-add-blog.php" method="POST" enctype="multipart/form-data">
