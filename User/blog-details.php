...
// Query untuk mengambil tiga artikel terbaru
$sql_latest_articles = "SELECT id, title, cover_image FROM blog ORDER BY created_at DESC LIMIT 3";
$result_latest_articles = $conn->query($sql_latest_articles);

// Simpan data artikel dalam array
$latest_articles = [];
if ($result_latest_articles->num_rows > 0) {
...
<body>
    ...
    <div class="blog-detail">
        ...
        <div class="col-lg-8">
            <h3 class="blog-detail-title"><?= htmlspecialchars($blog['title']); ?></h3>
            <?php if (!empty($blog['header_image'])): ?>
        <div class="blog-header-image">
            <img src="../Admin/<?= htmlspecialchars($blog['header_image']); ?>" alt="<?= htmlspecialchars($blog['title']); ?>" class="img-fluid">
        </div>
        <?php endif; ?>
        </div>
            <div class="col-lg-8">
                <!-- Konten Blog -->
                <p class="desc mt-4 mt-lg-7"><?= nl2br(htmlspecialchars($blog['content'])); ?></p>
            </div>
        </div>
        ...
        <div class="row justify-content-center">
            <div class="col-lg-8">
                ...
                <blockquote class="blog-detail-blockquote mt-6 mt-lg-10 mb-6 mb-lg-10">
                    <p class="desc">“<?= htmlspecialchars($blog['quote']); ?>”</p>
                    <span class="user-name">- <?= htmlspecialchars($blog['quote_author']); ?></span>
